...
/// More workers: faster (8 cores on my machine).
#[bench]
fn bench_large_parallel_plus(b: &mut Bencher) {
    let texts = all_texts(30);
    b.iter(|| parallel_letter_frequency::frequency(&texts, 8));
}
...
/// Following benches are custom ones meant to empirically find a suitable threshold.
#[bench]
fn bench_custom_1_par(b: &mut Bencher) {
    let texts = all_texts(1);
    b.iter(|| parallel_letter_frequency::frequency(&texts, 3));
}

#[bench]
fn bench_custom_1_seq(b: &mut Bencher) {
    let texts = all_texts(1);
    b.iter(|| frequency(&texts));
}

#[bench]
fn bench_custom_2_par(b: &mut Bencher) {
    let texts = all_texts(2);
    b.iter(|| parallel_letter_frequency::frequency(&texts, 3));
}

#[bench]
fn bench_custom_2_seq(b: &mut Bencher) {
    let texts = all_texts(2);
    b.iter(|| frequency(&texts));
}

#[bench]
fn bench_custom_3_par(b: &mut Bencher) {
    let texts = all_texts(3);
    b.iter(|| parallel_letter_frequency::frequency(&texts, 3));
}

#[bench]
fn bench_custom_3_seq(b: &mut Bencher) {
    let texts = all_texts(3);
    b.iter(|| frequency(&texts));
}

#[bench]
fn bench_custom_4_par(b: &mut Bencher) {
    let texts = all_texts(4);
    b.iter(|| parallel_letter_frequency::frequency(&texts, 3));
}

#[bench]
fn bench_custom_4_seq(b: &mut Bencher) {
    let texts = all_texts(4);
    b.iter(|| frequency(&texts));
}

#[bench]
fn bench_custom_5_par(b: &mut Bencher) {
    let texts = all_texts(5);
    b.iter(|| parallel_letter_frequency::frequency(&texts, 3));
}

#[bench]
fn bench_custom_5_seq(b: &mut Bencher) {
    let texts = all_texts(5);
    b.iter(|| frequency(&texts));
}
